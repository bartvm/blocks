...
def test_dataset_evaluators():
    ...
    data = [numpy.arange(1, 4, dtype=floatX).reshape(3, 1) / 7.0,
            numpy.arange(11, 14, dtype=floatX).reshape(3, 1) / 17.0]

    data_stream = IterableDataset(dict(X=data[0], Y=data[1])).get_example_stream()

    print data

    validator = DatasetEvaluator([CrossEntropy(requires=[X, Y]), CategoricalCrossEntropy().apply(X, Y), ])
